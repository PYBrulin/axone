...
def udp_multicast_listener(multicast_group: str, port: int, interface_ip: str = '0.0.0.0'):
    try:
        ...
        sock.bind(('', port))
        logging.info(f"Socket bound to {interface_ip}:{port}")
        ...
        logging.info(f"Joined multicast group {multicast_group} on interface {interface_ip}")
...
if __name__ == "__main__":
    ...
    udp_multicast_listener("224.1.1.1", int(sys.argv[1]) if len(sys.argv) > 1 else 5007, "192.168.144.1")
