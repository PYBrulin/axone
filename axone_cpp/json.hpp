            ...
            bool line_wrap = true) const
        {
            ...
            case Class::Object:
            {
                ...
                for (auto &p : *Internal.Map)
                {
                    ...
                    if (line_wrap)
                        s += "\n";
                    s += (pad + "\"" + p.first + "\"" + key_separator + p.second.dump(depth + 1, tab, item_separator, key_separator, line_wrap));
                    skip = false;
                }
                if (line_wrap)
                    s += "\n";
            ...
            case Class::Array:
            {
                ...
                for (auto &p : *Internal.List)
                {
                    if (!skip)
                        s += item_separator;
                    s += p.dump(depth + 1, tab, item_separator, key_separator, line_wrap);
                    skip = false;
                }
                s += "]";
                return s;
